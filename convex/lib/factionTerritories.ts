...
function getProgress(campaigns: CampaignProgressInput[], factionId: FactionId): number {
...
export function getZoneTerritory(zoneId: string, campaigns: CampaignProgressInput[]): ZoneTerritory {
  const homeFaction = (ZONE_FACTION_MAP[zoneId] ?? "neutre") as FactionId;
...
export function campaignRowsToInput(
  rows: { factionId: FactionId; progress: number; target: number; status: "active" | "completed" }[]
